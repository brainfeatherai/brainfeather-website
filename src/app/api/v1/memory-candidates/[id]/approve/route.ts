...
async function approveCandidate(
  ...
  try {
    const result = await approveMemoryCandidate(auth.userId, id);
    ...
    reportServerError(error, {
      operation: 'memory_candidate.approve',
      route: '/api/v1/memory-candidates/:id/approve',
      userId: auth.userId,
      resourceId: id,
    });
    return fail(500, 'Could not approve the memory candidate.');
  }
}

export const POST = withRequestTelemetry(
  'memory_candidate.approve',
  approveCandidate,
);
